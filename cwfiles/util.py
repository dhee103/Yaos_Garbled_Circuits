...
def generate_perms(n):
    return [padZeros('{0:b}'.format(x),n) for x in range(2**n)]
...
def find_input_values(inputs, wires):
    input_values = []
    for input in inputs:
        for wire in wires:
            if input == wire.source:
                input_values.append(wire.value)
                break
    return tuple(input_values)

# find wire whose source is same as 'source'
def find_wire(source, wires):
    for wire in wires:
        if wire.source == source:
            return wire

def print_output(perm, value, alice, bob, output):
    n = len(alice)
    print("Alice" + str(alice) + " = " + " ".join(list(perm[:n])) + " Bob" + str(bob) + " = " + " ".join(list(perm[n:])) + "  Outputs" + str(output) + " = " + str(value))
